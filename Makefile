...
### Zig
.PHONY: build-client-zig
build-client-zig:
	make build-client sdk_language=zig tmpdir=${TMP_DIR}
	# Note: Additional Zig-specific build steps would go here
	# The C-libcurl generator output will need Zig wrapper code

.PHONY: test-client-zig
test-client-zig: build-client-zig
	# Note: Zig testing would require implementing a Zig build system
	# and wrapping the C client library generated by OpenAPI Generator
	@echo "Zig SDK testing not yet implemented - requires Zig wrapper implementation"
